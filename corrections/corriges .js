...
`mongoimport --db us --collection companies --file companies.json``use us`;
...
db.companies.updateMany({ category_code: /advert/gi }, { $set: { likes: 0 } });
...
var allCompanies = db.companies.find({
...
allCompanies.forEach((oneCompany) => {
  if (
    oneCompany.offices[0].longitude != null &&
    oneCompany.offices[0].latitude != null
  ) {
